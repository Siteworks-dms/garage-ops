  ...
  return (
    <div style={{ background: "#161B22", border: `1px solid ${col}30`, borderTop: `3px solid ${col}`, borderRadius: 14, overflow: "hidden", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "20px 22px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", gap: 14 }}>
        ...
        {active.length > 0 && (
          <div style={{ background: "rgba(59,130,246,0.1)", border: "1px solid rgba(59,130,246,0.3)", borderRadius: 8, padding: "6px 12px", textAlign: "center" }}>
            <div style={{ fontSize: 11, color: "#6E7681", letterSpacing: "0.08em" }}>ON JOB</div>
            <div style={{ fontSize: 22, fontWeight: 700, color: "#3B82F6", lineHeight: 1.2 }}>{active.length}</div>
          </div>
        )}
      </div>
...
function TVDisplay() {
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#0D1117", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ color: "#6E7681", fontSize: 18, letterSpacing: "0.1em" }}>LOADING DISPLAY…</div>
    </div>
  );
    ...
    <div style={{ minHeight: "100vh", background: "#0D1117", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ height: 36, background: "#161B22", borderTop: "1px solid rgba(255,255,255,0.06)", overflow: "hidden", display: "flex", alignItems: "center", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, paddingLeft: 16, flexShrink: 0, borderRight: "1px solid rgba(255,255,255,0.08)", paddingRight: 16, height: "100%" }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 700, color: "#10B981", letterSpacing: "0.1em" }}>LIVE</span>
        </div>
            ...
            return (
              <div key={m.id} onClick={() => handleSelectMech(m)} style={{ padding: "12px 18px", cursor: "pointer", background: isActive ? "rgba(245,158,11,0.07)" : "transparent", borderLeft: isActive ? "3px solid #F59E0B" : "3px solid transparent", display: "flex", alignItems: "center", gap: 10, transition: "background .1s" }}
                ...
                <div style={{ position: "relative" }}>
                  ...
                  {unread > 0 && <div style={{ position: "absolute", top: -4, right: -4, background: "#EF4444", borderRadius: "50%", width: 16, height: 16, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 700, color: "#fff", border: "2px solid #161B22" }}>{unread > 9 ? "9+" : unread}</div>}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        {!selectedMech ? (
          <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 12, color: "#484f58" }}>
            ...
            <div style={{ fontSize: 14 }}>{isManager ? "Select a mechanic to start messaging" : "Select a conversation"}</div>
          </div>
        ) : (
          <>
            ...
            <div style={{ padding: "12px 16px", borderTop: "1px solid rgba(255,255,255,0.07)", display: "flex", gap: 10, alignItems: "flex-end" }}>
              <textarea value={body} onChange={e => setBody(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSend(); } }} placeholder={`Message ${selectedMech.full_name}… (Enter to send)`} rows={2} style={{ flex: 1, fontSize: 14, padding: "10px 12px", minHeight: 52, maxHeight: 120 }} />
              <Btn variant="primary" onClick={handleSend} disabled={sending || !body.trim()} style={{ padding: "12px 18px", height: 52, flexShrink: 0 }}>{sending ? <Spinner size={14} /> : "SEND"}</Btn>
            </div>
          </>
        )}
      </div>
      ...
      <div className="slide-in" style={{ width: "100%", maxWidth: 420, background: "#161B22", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 18, overflow: "hidden", boxShadow: "0 32px 100px rgba(0,0,0,0.6)" }}>
        ...
        <div style={{ padding: "36px 36px 32px" }}>
          ...
          <div style={{ marginTop: 22, background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10, padding: "14px 16px" }}>
            ...
            {demo.map(d => (<div key={d.email} onClick={() => { setEmail(d.email); setPassword(d.pass); }} style={{ display: "flex", alignItems: "center", gap: 8, padding: "5px 0", cursor: "pointer", fontSize: 12 }} onMouseEnter={e => e.currentTarget.style.opacity = ".75"} onMouseLeave={e => e.currentTarget.style.opacity = "1"}><span style={{ fontSize: 10, fontWeight: 700, color: d.color, background: d.color + "18", border: `1px solid ${d.color}35`, borderRadius: 4, padding: "1px 6px", minWidth: 60, textAlign: "center" }}>{d.label.toUpperCase()}</span><span className="mono" style={{ color: "#8B949E", fontSize: 12 }}>{d.email} / {d.pass}</span></div>))}
...
function TopNav({ user, onLogout, unreadCount = 0 }) {
  const openTV = () => window.open(window.location.origin + "?tv=1", "_blank");
  return (
...
function AddMechanicModal({ onClose, onCreated }) {
  ...
  const [errs, setErrs] = useState({}); const [saving, setSaving] = useState(false); const [apiErr, setApiErr] = useState(""); const [success, setSuccess] = useState("");
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));
  const validate = () => { const e = {}; if (!form.name.trim()) e.name = "Required"; if (!form.email.trim()) e.email = "Required"; if (!form.password) e.password = "Required"; else if (form.password.length < 6) e.password = "Min 6 characters"; if (form.confirm !== form.password) e.confirm = "Passwords do not match"; return e; };
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.72)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, zIndex: 200, backdropFilter: "blur(3px)" }}>
      <div className="slide-in" style={{ background: "#1C2333", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 16, width: "100%", maxWidth: 480 }}>
        <div style={{ padding: "20px 26px", borderBottom: "1px solid rgba(255,255,255,0.07)", display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
          <div><div style={{ fontSize: 17, fontWeight: 700, letterSpacing: "0.05em" }}>ADD MECHANIC ACCOUNT</div><div style={{ fontSize: 12, color: "#8B949E", marginTop: 3 }}>Creates a new login for a mechanic</div></div>
        ...
        <div style={{ padding: "24px 26px", display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div><FieldLabel required>Email</FieldLabel><input type="email" value={form.email} onChange={e => set("email", e.target.value)} placeholder="mechanic@yourgarage.com" /><FieldErr msg={errs.email} /></div>
        ...
        <div style={{ padding: "16px 26px", borderTop: "1px solid rgba(255,255,255,0.07)", display: "flex", gap: 10, justifyContent: "flex-end" }}>
          ...
          <Btn variant="green" onClick={handleCreate} disabled={saving}>{saving ? <span style={{ display: "flex", alignItems: "center", gap: 8 }}><Spinner size={14} /> CREATING…</span> : "CREATE ACCOUNT"}</Btn>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.72)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, zIndex: 200, backdropFilter: "blur(3px)" }}>
      <div className="slide-in" style={{ background: "#1C2333", border: "1px solid rgba(239,68,68,0.25)", borderRadius: 16, padding: "32px 32px 28px", width: "100%", maxWidth: 460 }}>
        <div style={{ width: 52, height: 52, borderRadius: 14, background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 20 }}>
          <svg width="26" height="26" viewBox="0 0 24 24" fill="none"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2" stroke="#EF4444" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><circle cx="9" cy="7" r="4" stroke="#EF4444" strokeWidth="2"/><line x1="18" y1="8" x2="23" y2="13" stroke="#EF4444" strokeWidth="2" strokeLinecap="round"/><line x1="23" y1="8" x2="18" y2="13" stroke="#EF4444" strokeWidth="2" strokeLinecap="round"/></svg>
        </div>
        <div style={{ fontSize: 19, fontWeight: 700, marginBottom: 10 }}>Remove Mechanic Account</div>
        <div style={{ color: "#8B949E", fontSize: 14, lineHeight: 1.8, marginBottom: 16 }}>Remove <strong style={{ color: "#E6EDF3" }}>{mechanic.full_name}</strong> from the system.</div>
        {orderCount > 0 && <div style={{ background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: 8, padding: "10px 14px", fontSize: 13, color: "#D97706", marginBottom: 16, lineHeight: 1.6 }}>⚠ {orderCount} order{orderCount !== 1 ? "s" : ""} will be set to Unassigned.</div>}
        <div style={{ background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.15)", borderRadius: 8, padding: "10px 14px", fontSize: 12, color: "#8B949E", marginBottom: 24, lineHeight: 1.6 }}>This action <strong style={{ color: "#F87171" }}>cannot be undone</strong>.</div>
...
function MechanicsPanel({ mechanics, orders, onAdd, onDelete, loading }) {
  ...
  return (
    <div>
      ...
      {loading ? (<div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 60, gap: 12, color: "#8B949E" }}><Spinner /> Loading team…</div>)
        : mechanics.length === 0 ? (
              ...
              return (
                <div key={m.id} style={{ background: "#1C2333", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 12, padding: "20px 20px 16px", transition: "border-color .15s" }} onMouseEnter={e => e.currentTarget.style.borderColor = "rgba(255,255,255,0.12)"} onMouseLeave={e => e.currentTarget.style.borderColor = "rgba(255,255,255,0.06)"}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 15, fontWeight: 700, marginBottom: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.full_name}</div><div style={{ fontSize: 12, color: "#6E7681", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.email ?? "—"}</div></div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8, marginBottom: 16 }}>
                    {[{ label: "Total", value: total, color: "#E6EDF3" }, { label: "Pending", value: pending, color: "#F59E0B" }, { label: "Active", value: inProg, color: "#3B82F6" }, { label: "Done", value: done, color: "#10B981" }].map(s => (
                      <div key={s.label} style={{ background: "rgba(255,255,255,0.03)", borderRadius: 8, padding: "8px 6px", textAlign: "center" }}><div style={{ fontSize: 20, fontWeight: 700, color: s.color, lineHeight: 1 }}>{s.value}</div><div style={{ fontSize: 10, color: "#555d65", marginTop: 4, letterSpacing: "0.06em" }}>{s.label.toUpperCase()}</div></div>
                    ))}
                  </div>
  ...
  return (
    <div style={{ background: "#0D1117", minHeight: "100vh" }}>
      ...
      <div className="page-pad" style={{ padding: "28px 24px", maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ marginBottom: 6 }}><h1 style={{ fontSize: 24, fontWeight: 700, letterSpacing: "0.05em" }}>MY DASHBOARD</h1><p style={{ fontSize: 13, color: "#8B949E", marginTop: 4 }}>{user.full_name} · Mechanic</p></div>
        <div style={{ borderBottom: "1px solid rgba(255,255,255,0.06)", marginBottom: 24, marginTop: 16 }}><TAB id="orders" label="MY ORDERS" /><TAB id="messages" label="MESSAGES" badge={unreadTotal} /></div>
        <ErrBanner msg={dbError} />
        {activeTab === "orders" && (
          <>

...
function Btn({ children, variant = "ghost", onClick, disabled, style: sx = {} }) {
  ...
  return (
    <button onClick={onClick} disabled={disabled} style={{ ...variants[variant], borderRadius: 8, padding: "9px 18px", fontSize: 13, fontWeight: 700, cursor: disabled ? "not-allowed" : "pointer", opacity: disabled ? .5 : 1, letterSpacing: "0.06em", transition: "opacity .15s, filter .15s", fontFamily: "'Rajdhani', sans-serif", whiteSpace: "nowrap", ...sx }}
...
function LoginScreen({ onLogin }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (pe) { setError("Could not load your profile. Contact your manager."); setLoading(false); return; }
...
function AddMechanicModal({ onClose, onCreated }) {
  ...
  const handleCreate = async () => {
    ...
    // Use isolated authClient — keeps manager session intact
    ...
    // Immediately sign out of the secondary client
      ...
      <div className="slide-in" style={{ background: "#1C2333", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 16, width: "100%", maxWidth: 480 }}>
        ...
        <div style={{ padding: "24px 26px", display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ background: "rgba(245,158,11,0.05)", border: "1px solid rgba(245,158,11,0.15)", borderRadius: 8, padding: "10px 14px", fontSize: 12, color: "#8B949E", lineHeight: 1.6 }}>
            The mechanic logs in at the same URL with their email and this password. Role is automatically set to <strong style={{ color: "#F59E0B" }}>Mechanic</strong> — no manager access.
          </div>
        </div>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.72)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, zIndex: 200, backdropFilter: "blur(3px)" }}>
      <div className="slide-in" style={{ background: "#1C2333", border: "1px solid rgba(239,68,68,0.25)", borderRadius: 16, padding: "32px 32px 28px", width: "100%", maxWidth: 460 }}>
        ...
        <div style={{ color: "#8B949E", fontSize: 14, lineHeight: 1.8, marginBottom: 16 }}>
          You are about to remove <strong style={{ color: "#E6EDF3" }}>{mechanic.full_name}</strong> from the system.
        </div>

        {orderCount > 0 && (
          <div style={{ background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: 8, padding: "10px 14px", fontSize: 13, color: "#D97706", marginBottom: 16, lineHeight: 1.6 }}>
            ⚠ This mechanic has <strong>{orderCount}</strong> assigned work order{orderCount !== 1 ? "s" : ""}. They will be set to <strong>Unassigned</strong>.
          </div>
        )}

        <div style={{ background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.15)", borderRadius: 8, padding: "10px 14px", fontSize: 12, color: "#8B949E", marginBottom: 24, lineHeight: 1.6 }}>
          Their profile and login access will be permanently removed. This action <strong style={{ color: "#F87171" }}>cannot be undone</strong>.
        </div>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
          ...
          <Btn variant="danger" onClick={onConfirm} disabled={deleting}>
            {deleting ? <span style={{ display: "flex", alignItems: "center", gap: 8 }}><Spinner size={14} /> REMOVING…</span> : "REMOVE MECHANIC"}
          </Btn>
        </div>
      </div>
    </div>
            ...
            return (
              ...
              >
                ...
                <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 14 }}>
                  <Btn variant="red" onClick={() => onDelete(m)} style={{ width: "100%", padding: "8px", fontSize: 12 }}>
                    REMOVE ACCOUNT
                  </Btn>
                </div>
              </div>
      ...
      <div className="slide-in" style={{ background: "#1C2333", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 16, width: "100%", maxWidth: 620, maxHeight: "92vh", overflow: "auto" }}>
        ...
        <div style={{ padding: "24px 26px" }}>
          <div className="modal-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            ...
            <div style={{ gridColumn: "1 / -1" }}><FieldLabel required>Task Description</FieldLabel><textarea value={form.task} onChange={e => set("task", e.target.value)} placeholder="Describe the work to be performed in detail..." rows={4} /><FieldErr msg={errs.task} /></div>
          </div>
        </div>
        <div style={{ padding: "16px 26px", borderTop: "1px solid rgba(255,255,255,0.07)", display: "flex", gap: 10, justifyContent: "flex-end" }}>
          <Btn variant="ghost" onClick={onClose} disabled={saving}>CANCEL</Btn>
      ...
      <div className="slide-in" style={{ background: "#1C2333", border: "1px solid rgba(239,68,68,0.25)", borderRadius: 16, padding: "32px 32px 28px", width: "100%", maxWidth: 450 }}>
        <div style={{ width: 52, height: 52, borderRadius: 14, background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 20 }}>
          <svg width="26" height="26" viewBox="0 0 24 24" fill="none">
            ...
            <path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" stroke="#EF4444" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" fill="none"/>
          </svg>
        </div>
...
function WorkOrderTable({ orders, mechanics, onEdit, onDelete, onCreate, loading }) {
  ...
  const TH = ({ label, field, style: sx = {} }) => (
    <th onClick={field ? () => toggleSort(field) : undefined} style={{ padding: "12px 16px", textAlign: "left", fontSize: 10, fontWeight: 700, color: "#6E7681", letterSpacing: "0.1em", whiteSpace: "nowrap", cursor: field ? "pointer" : "default", userSelect: "none", ...sx }}
      ...
      <div className="page-pad" style={{ padding: "28px 24px", maxWidth: 1440, margin: "0 auto" }}>
        ...
        {activeTab === "orders" && (
          <>
            ...
            <WorkOrderTable orders={orders} mechanics={mechanics} loading={loading}
        ...
        {activeTab === "team" && (
          <MechanicsPanel mechanics={mechanics} orders={orders} loading={loading}
            onAdd={() => setShowAddMech(true)}
            onDelete={m => setSelectedMech(m)}
          />
        )}
      </div>

      {(modal === "create" || modal === "edit") && (
        <OrderModal mode={modal} order={selected} mechanics={mechanics} onSave={handleSave} onClose={closeOrder} saving={saving} />
      )}
      {modal === "delete" && (
        <DeleteOrderModal order={selected} onConfirm={handleDeleteOrder} onClose={closeOrder} deleting={deleting} />
      )}
      {showAddMech && (
        <AddMechanicModal onClose={() => setShowAddMech(false)} onCreated={() => loadMechanics()} />
      )}
      {selectedMech && (
        <DeleteMechanicModal
          mechanic={selectedMech}
          orderCount={orders.filter(o => o.mechanic_id === selectedMech.id).length}
          onConfirm={handleDeleteMechanic}
          onClose={() => setSelectedMech(null)}
          deleting={deletingMech}
        />
      )}
    </div>
  );
...
function MechanicDashboard({ user, onLogout }) {
  ...
  useEffect(() => {
    setLoading(true);
    supabase.from("work_orders").select("*").eq("mechanic_id", user.id).order("created_at", { ascending: false })
      .then(({ data, error }) => { if (error) setDbError(error.message); else setOrders(data ?? []); setLoading(false); });
  }, []);
      ...
      <div className="page-pad" style={{ padding: "28px 24px", maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ marginBottom: 24 }}>
          ...
          <p style={{ fontSize: 13, color: "#8B949E", marginTop: 4 }}>
            {loading ? "Loading…" : orders.length > 0 ? `${orders.length} order${orders.length !== 1 ? "s" : ""} assigned · Update status as you work` : "No orders assigned yet — check back later"}
          </p>
        </div>
        <ErrBanner msg={dbError} />
